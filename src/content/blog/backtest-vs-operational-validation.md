...
This is expensive. Some controls can only be tested in ways that are operationally disruptive or that require the production team to deliberately induce a failure condition. But the alternative — discovering during an examination that a key control has never been exercised — is more expensive in a different way.
