...
modDatetime: 2026-03-18T12:10:00+08:00
...
In AML, a Suspicious Transaction Report (STR) means someone decided to file. It does not mean the customer is a criminal. An STR that was never filed doesn't mean the customer is clean — it means nobody looked hard enough, or the analyst had a bad day, or the case sat in a queue too long.

This sounds obvious when stated directly. But watch what happens when you build a model on these labels.

The model learns to predict STR filings — human process decisions — not criminal activity. When we say "the model has a 0.5% miss rate in the low-risk bucket," we mean 0.5% of *filed STRs* fell below the threshold. We have no idea how many actual criminals the model scored as low-risk, because "actual criminal" isn't in the dataset. It never was.

This isn't an AML-specific problem. It's everywhere supervised learning touches human institutions:
...
This matters most when the model's predictions feed back into the same process that generated the labels. In AML: the model scores alerts, analysts review the high-scoring ones, their STR decisions become next year's training data. The loop is closed. The model converges on predicting what analysts do, which is all it was ever trained to do. Whether this has anything to do with catching criminals is an article of faith, not an empirical finding.

The uncomfortable implication: **evaluating the model against historical labels tells you how well it mimics the process, not how well it achieves the goal.** A model that perfectly predicts analyst STR decisions is perfectly useless if analysts are systematically missing a category of criminal. And you'd never know from the metrics.

What breaks the loop? External signals — law enforcement referrals, correspondent bank notifications, adverse media. These come from outside the process and occasionally reveal that someone the process cleared was actually a problem. But these signals are rare, slow, and unsystematic. They're the only window into ground truth, and the window is tiny.
